...
public class EventHandlers implements RequestHandler<ScheduledEvent, String> {
    ...
    private void processShipmentUpdates(final LambdaLogger logger) throws InterruptedException {
        ...
        final AmazonS3 s3Client = EventHandlers.getS3Client();
        ...
        EventHandlers.getS3Client().putObject(Constants.SUMMARY_BUCKET, summaryUpdateName, latestStatusForTrackingNumber.toString());
        ...
        if (EventHandlers.getS3Client().doesObjectExist(Constants.SUMMARY_BUCKET, summaryUpdateName)) {
